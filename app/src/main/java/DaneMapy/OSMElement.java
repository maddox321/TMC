...
import com.maddox.tmc.DrogaTyp;
import com.maddox.tmc.MapBuilder;
import com.maddox.tmc.Polygon;
import com.maddox.tmc.Punkt;
...
public class OSMElement {
    ...
    public MapItem convert() {
        ...
        item.id 	 = this.id;
        item.nameId  = 0;
        item.type    = (int)(Integer) DrogaTyp.presets.get(this.getType());
        ...
        for (int nodeIdx = 0; nodeIdx < numNodes; ++nodeIdx) {
            ...
            Node nd = (Node) MapBuilder.nodeMap.get(this.nodeRefs.get(nodeIdx));
        ...
        if ((item.flags & SHAPE_AREA_MASK) == SHAPE_AREA_MASK) {
            ...
            Punkt[] vertices = new Punkt[num];

            for(int j = 0; j < num; j++) {

                Node nd  = (Node)MapBuilder.nodeMap.get(this.nodeRefs.get(j));
                vertices[j] = new Punkt(nd.x, nd.y);
            }

            Polygon poly = new Polygon(vertices);
            LinkedList<Punkt> triangles = poly.triangulate();
            ...
            for (int i = 0; i < item.numNodes; i ++) {
                ...
                Node node    = MapBuilder.nodeMap.get(nodeRef);
                item.nodes[c]   = node.x;
            ...
            if (this.getType().contains("highway")) {
                ...
                for(nodeIdx = 0; nodeIdx < numNodes; ++nodeIdx) {
                    ...
                    Node nd   = (Node)MapBuilder.nodeMap.get(nodeRef);
